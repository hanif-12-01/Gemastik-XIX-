...
start_idx = c.find('  const OrdersView = () => (')
end_idx = c.find('  // AI Digitization View')
                ...
                <tr key={order.id} className=\"hover:bg-gray-50 transition-colors\">
                  ...
                  <td className=\"px-4 py-4\">
                    {assignedOrders[order.id] ? (
                      ...
                      <button onClick={() => handleAssign(order.id)} disabled={assigningId === order.id} className={\	ext-sm font-medium px-4 py-2 rounded-lg flex items-center justify-center gap-2 w-max transition-all \\}>
...
\"\"\"
    ...
    new_c = c.replace(old_block, replacement)

...
export const MitraLoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== 'mitra') {
        setError('Akun ini bukan akun Mitra Penjahit.')
        return
      }

      const status = user.mitraProfile?.verificationStatus || 'pending_verification'
      if (status === 'approved') {
        navigate(ROUTES.MITRA.DASHBOARD)
      } else {
        navigate(ROUTES.MITRA.VERIFICATION_STATUS)
      }
    } catch (err: any) {
    ...
    <div style={{ maxWidth: '440px', margin: '3rem auto' }}>
      ...
      <Card style={{ padding: '2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          <Badge variant="info" className="mb-2">Portal Mitra Penjahit</Badge>
          <h1 style={{ fontSize: '1.5rem', color: '#FFF' }}>Masuk Login Mitra</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', marginTop: '1rem' }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', color: 'var(--color-text-muted)', marginBottom: '0.375rem' }}>Email Mitra</label>
            <Input
              type="email"
              value={email}
          ...
          <Button type="submit" variant="primary" disabled={loading} style={{ width: '100%', padding: '0.75rem' }}>
            {loading ? 'Memverifikasi...' : 'Masuk Portal Mitra'}
          </Button>
        </form>

        <div style={{ borderTop: '1px solid var(--color-border)', marginTop: '1.5rem', paddingTop: '1rem', fontSize: '0.85rem', color: 'var(--color-text-muted)', textAlign: 'center' }}>
